    ...
    <style>
        ...
        *{box-sizing:border-box}
        ...
        .hero{
            ...
            display:flex;
            align-items:center;
            background-image:url("{{ asset('images/home-hero.jpeg') }}");
...
<body>
    <header class="site-header">
        <div class="nav">
            <div class="brand" style="display:flex;align-items:center;gap:10px">
                <img src="{{ asset('images/logo.jpeg') }}" alt="SOQIA Innovative Environmental Solutions" style="height:20px;width:auto;border-radius:4px">
    ...
    <section class="hero">
        <video class="hero-video" src="{{ asset('images/hero.mp4') }}" autoplay muted loop playsinline></video>
        <div class="hero-content container">
            <h1 class="hero-title">🌾 {{ __('home.hero.title') }}</h1>
            <p class="hero-sub">{{ __('home.hero.subtitle') }}</p>
            <div class="hero-actions">
                <button class="btn btn-green">{{ __('home.hero.buttons.explore') }}</button>
                <button class="btn btn-blue">{{ __('home.hero.buttons.services') }}</button>
                <button class="btn btn-orange">{{ __('home.hero.buttons.consult') }}</button>
            </div>
        </div>
    </section>
    ...
    <section class="container" style="margin-top:24px">
        <h2 style="font-size:22px;font-weight:800;color:#153015;margin-bottom:10px">{{ app()->getLocale() === 'ar' ? 'مختارات توعوية' : 'Awareness Highlights' }}</h2>
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px">
            @foreach($contents as $item)
                <div style="background:#fff;border:1px solid #e6e6e6;border-radius:12px;overflow:hidden">
                    <img src="{{ $item->image_path ? asset($item->image_path) : asset('images/farm1.jpg') }}" alt="" style="height:140px;width:100%;object-fit:cover">
                    <div style="padding:12px">
                        <div style="font-weight:700;color:#203020">{{ $item->title }}</div>
                        @if($item->body)
                            <div style="margin-top:6px;color:#555;font-size:13px">{{ \Illuminate\Support\Str::limit(strip_tags($item->body), 120) }}</div>
                        @endif
                        <div style="margin-top:8px;display:flex;justify-content:space-between;align-items:center;font-size:12px;color:#777">
                            ...
                            <span style="color:#FF9800">{{ app()->getLocale() === 'ar' ? 'اقرأ المزيد' : 'Read more' }}</span>
                        </div>
                    </div>
                </div>
            @endforeach
    ...
    <section class="tabs">
        <div class="container">
            <div class="tabs-header" id="tabsHeader">
                <button class="tab-btn active" data-tab="products">{{ __('home.tabs.labels.products') }}</button>
                <button class="tab-btn" data-tab="services">{{ __('home.tabs.labels.services') }}</button>
                <button class="tab-btn" data-tab="consult">{{ __('home.tabs.labels.consult') }}</button>
                <div class="tab-indicator" id="tabIndicator"></div>
            </div>
            <div class="tab-panels">
                <div class="panel active" id="panel-products">
                    <div class="panel-head">
                        <h2 class="panel-title">📦 {{ __('home.tabs.products.title') }}</h2>
                    </div>
                    <p class="panel-desc">{{ __('home.tabs.products.description') }}</p>
                    <div class="grid">
                        <div class="card">
                            <div class="thumb" style="background-image:url('{{ asset('images/product-1.jpg') }}')"></div>
                            <div class="card-body">
                                <div style="display:flex;align-items:center;margin-bottom:6px">
                                    <div class="icon">💧</div>
                                    <h3 class="card-title">{{ __('home.tabs.products.items.1.title') }}</h3>
                                </div>
                                <p class="card-text">{{ __('home.tabs.products.items.1.description') }}</p>
                                <div class="card-meta">
                                    <span class="price">{{ __('home.tabs.products.items.1.price') }}</span>
                                    <button class="details-btn">{{ __('home.tabs.products.items.details') }}</button>
                                </div>
                            </div>
                        </div>
                        <div class="card">
                            <div class="thumb" style="background-image:url('{{ asset('images/product-2.jpg') }}')"></div>
                            <div class="card-body">
                                <div style="display:flex;align-items:center;margin-bottom:6px">
                                    <div class="icon">🔧</div>
                                    <h3 class="card-title">{{ __('home.tabs.products.items.2.title') }}</h3>
                                </div>
                                <p class="card-text">{{ __('home.tabs.products.items.2.description') }}</p>
                                <div class="card-meta">
                                    <span class="price">{{ __('home.tabs.products.items.2.price') }}</span>
                                    <button class="details-btn">{{ __('home.tabs.products.items.details') }}</button>
                                </div>
                            </div>
                        </div>
                        <div class="card">
                            <div class="thumb" style="background-image:url('{{ asset('images/product-3.jpg') }}')"></div>
                            <div class="card-body">
                                <div style="display:flex;align-items:center;margin-bottom:6px">
                                    <div class="icon">📡</div>
                                    <h3 class="card-title">{{ __('home.tabs.products.items.3.title') }}</h3>
                                </div>
                                <p class="card-text">{{ __('home.tabs.products.items.3.description') }}</p>
                                <div class="card-meta">
                                    <span class="price">{{ __('home.tabs.products.items.3.price') }}</span>
                                    <button class="details-btn">{{ __('home.tabs.products.items.details') }}</button>
                                </div>
                            </div>
                        </div>
                ...
                <div class="panel" id="panel-services">
                    <div class="panel-head">
                        <h2 class="panel-title">🔧 {{ __('home.tabs.services.title') }}</h2>
                    </div>
                    <p class="panel-desc">{{ __('home.tabs.services.description') }}</p>
                    <div class="svc-grid">
                        <div class="svc-card">
                            <div class="svc-head">
                                <div class="svc-icon green">🏗️</div>
                                <h3 class="svc-title">{{ __('home.tabs.services.items.1.title') }}</h3>
                            </div>
                            <p class="svc-desc">{{ __('home.tabs.services.items.1.description') }}</p>
                            <ul class="bullets">
                                <li>✅ {{ __('home.tabs.services.items.1.points.1') }}</li>
                                <li>✅ {{ __('home.tabs.services.items.1.points.2') }}</li>
                                <li>✅ {{ __('home.tabs.services.items.1.points.3') }}</li>
                            </ul>
                            <div class="svc-meta">
                                <span class="svc-price">{{ __('home.tabs.services.items.1.price') }}</span>
                                <button class="svc-cta">{{ __('home.tabs.services.items.cta') }}</button>
                            </div>
                        </div>
                        <div class="svc-card">
                            <div class="svc-head">
                                <div class="svc-icon blue">🔗</div>
                                <h3 class="svc-title">{{ __('home.tabs.services.items.2.title') }}</h3>
                            </div>
                            <p class="svc-desc">{{ __('home.tabs.services.items.2.description') }}</p>
                            <ul class="bullets">
                                <li>✅ {{ __('home.tabs.services.items.2.points.1') }}</li>
                                <li>✅ {{ __('home.tabs.services.items.2.points.2') }}</li>
                                <li>✅ {{ __('home.tabs.services.items.2.points.3') }}</li>
                            </ul>
                            <div class="svc-meta">
                                <span class="svc-price">{{ __('home.tabs.services.items.2.price') }}</span>
                                <button class="svc-cta">{{ __('home.tabs.services.items.cta') }}</button>
                            </div>
                        </div>
                        <div class="svc-card">
                            <div class="svc-head">
                                ...
                                <h3 class="svc-title">{{ __('home.tabs.services.items.3.title') }}</h3>
                            </div>
                            <p class="svc-desc">{{ __('home.tabs.services.items.3.description') }}</p>
                            <ul class="bullets">
                                <li>✅ {{ __('home.tabs.services.items.3.points.1') }}</li>
                                <li>✅ {{ __('home.tabs.services.items.3.points.2') }}</li>
                                <li>✅ {{ __('home.tabs.services.items.3.points.3') }}</li>
                            </ul>
                            <div class="svc-meta">
                                <span class="svc-price">{{ __('home.tabs.services.items.3.price') }}</span>
                                <button class="svc-cta">{{ __('home.tabs.services.items.cta') }}</button>
                            </div>
                        </div>
                    </div>
                ...
                <div class="panel" id="panel-consult">
                    <div class="panel-head">
                        <h2 class="panel-title">📋 {{ __('home.tabs.consult.title') }}</h2>
                    </div>
                    <p class="panel-desc">{{ __('home.tabs.consult.description') }}</p>
                    <div class="cnsl-grid">
                        <div class="cnsl-card">
                            <div class="cnsl-head">
                                <div class="cnsl-icon green">📈</div>
                                <h3 class="cnsl-title">{{ __('home.tabs.consult.items.1.title') }}</h3>
                            </div>
                            <p class="cnsl-desc">{{ __('home.tabs.consult.items.1.description') }}</p>
                            <ul class="includes">
                                <li>💧 {{ __('home.tabs.consult.items.1.includes.1') }}</li>
                                <li>🌾 {{ __('home.tabs.consult.items.1.includes.2') }}</li>
                                <li>💰 {{ __('home.tabs.consult.items.1.includes.3') }}</li>
                                <li>⏱️ {{ __('home.tabs.consult.items.1.includes.4') }}</li>
                            </ul>
                            <div class="cnsl-meta">
                                <div class="cnsl-left">
                                    <span>{{ __('home.tabs.consult.items.1.duration') }}</span>
                                    <span class="cnsl-price">{{ __('home.tabs.consult.items.1.price') }}</span>
                                </div>
                                <button class="cnsl-cta">{{ __('home.tabs.consult.items.cta') }}</button>
                            </div>
                        </div>
                        <div class="cnsl-card">
                            <div class="cnsl-head">
                                <div class="cnsl-icon blue">🔧</div>
                                <h3 class="cnsl-title">{{ __('home.tabs.consult.items.2.title') }}</h3>
                            </div>
                            <p class="cnsl-desc">{{ __('home.tabs.consult.items.2.description') }}</p>
                            <ul class="includes">
                                <li>🔍 {{ __('home.tabs.consult.items.2.includes.1') }}</li>
                                <li>📊 {{ __('home.tabs.consult.items.2.includes.2') }}</li>
                                <li>💡 {{ __('home.tabs.consult.items.2.includes.3') }}</li>
                                <li>📋 {{ __('home.tabs.consult.items.2.includes.4') }}</li>
                            </ul>
                            <div class="cnsl-meta">
                                <div class="cnsl-left">
                                    <span>{{ __('home.tabs.consult.items.2.duration') }}</span>
                                    <span class="cnsl-price">{{ __('home.tabs.consult.items.2.price') }}</span>
                                </div>
                                <button class="cnsl-cta">{{ __('home.tabs.consult.items.cta') }}</button>
                            </div>
                        </div>
                        <div class="cnsl-card">
                            <div class="cnsl-head">
                                ...
                                <h3 class="cnsl-title">{{ __('home.tabs.consult.items.3.title') }}</h3>
                            </div>
                            <p class="cnsl-desc">{{ __('home.tabs.consult.items.3.description') }}</p>
                            <ul class="includes">
                                <li>♻️ {{ __('home.tabs.consult.items.3.includes.1') }}</li>
                                <li>💧 {{ __('home.tabs.consult.items.3.includes.2') }}</li>
                                <li>🌍 {{ __('home.tabs.consult.items.3.includes.3') }}</li>
                                <li>📊 {{ __('home.tabs.consult.items.3.includes.4') }}</li>
                            </ul>
                            <div class="cnsl-meta">
                                <div class="cnsl-left">
                                    <span>{{ __('home.tabs.consult.items.3.duration') }}</span>
                                    <span class="cnsl-price">{{ __('home.tabs.consult.items.3.price') }}</span>
                                </div>
                                <button class="cnsl-cta">{{ __('home.tabs.consult.items.cta') }}</button>
                            </div>
                        </div>
        ...
        <div class="container">
            <div class="stat">
                ...
                <div>
                    ...
                    <div class="stat-sub">{{ __('home.stats.items.1') }}</div>
                </div>
            </div>
            <div class="stat">
                <div class="stat-icon">👥</div>
                <div>
                    ...
                    <div class="stat-sub">{{ __('home.stats.items.2') }}</div>
                </div>
            </div>
            <div class="stat">
                <div class="stat-icon">💧</div>
                <div>
                    ...
                    <div class="stat-sub">{{ __('home.stats.items.3') }}</div>
                </div>
            </div>
            <div class="stat">
                <div class="stat-icon">⏳</div>
                <div>
                    ...
                    <div class="stat-sub">{{ __('home.stats.items.4') }}</div>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="about" style="padding:40px 0">
        <div class="container" style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
            <div style="background:#fff;border:1px solid #e6e6e6;border-radius:14px;padding:20px">
                <div style="font-size:20px;font-weight:800;color:#153015">{{ __('home.sections.about.title') }}</div>
                <p style="margin-top:8px;color:#555">{{ __('home.sections.about.desc') }}</p>
            </div>
            <div style="background:#fff;border:1px solid #e6e6e6;border-radius:14px;padding:20px">
                <div style="font-size:20px;font-weight:800;color:#153015">{{ __('home.sections.story.title') }}</div>
                <p style="margin-top:8px;color:#555">{{ __('home.sections.story.desc') }}</p>
            </div>
        </div>
    </section>
    <section class="reviews" style="padding:20px 0">
        <div class="container">
            <div style="font-size:20px;font-weight:800;color:#153015;margin-bottom:12px">{{ __('home.sections.reviews.title') }}</div>
            <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px">
                <div style="background:#fff;border:1px solid #e6e6e6;border-radius:14px;padding:16px">
                    <div style="font-weight:700;color:#1e2a1e">{{ __('home.sections.reviews.items.1.name') }}</div>
                    <div style="margin-top:6px;color:#555">{{ __('home.sections.reviews.items.1.text') }}</div>
                </div>
                <div style="background:#fff;border:1px solid #e6e6e6;border-radius:14px;padding:16px">
                    <div style="font-weight:700;color:#1e2a1e">{{ __('home.sections.reviews.items.2.name') }}</div>
                    <div style="margin-top:6px;color:#555">{{ __('home.sections.reviews.items.2.text') }}</div>
                </div>
                <div style="background:#fff;border:1px solid #e6e6e6;border-radius:14px;padding:16px">
                    <div style="font-weight:700;color:#1e2a1e">{{ __('home.sections.reviews.items.3.name') }}</div>
                    <div style="margin-top:6px;color:#555">{{ __('home.sections.reviews.items.3.text') }}</div>
                </div>
            </div>
        </div>
    </section>
    <section class="achievements" style="padding:20px 0">
        <div class="container">
            <div style="font-size:20px;font-weight:800;color:#153015;margin-bottom:12px">{{ __('home.sections.achievements.title') }}</div>
            <div style="display:flex;gap:12px;flex-wrap:wrap">
                <div style="flex:1;min-width:200px;background:#fff;border:1px solid #e6e6e6;border-radius:14px;padding:16px;text-align:center;color:#1e2a1e">{{ __('home.sections.achievements.items.1') }}</div>
                <div style="flex:1;min-width:200px;background:#fff;border:1px solid #e6e6e6;border-radius:14px;padding:16px;text-align:center;color:#1e2a1e">{{ __('home.sections.achievements.items.2') }}</div>
                <div style="flex:1;min-width:200px;background:#fff;border:1px solid #e6e6e6;border-radius:14px;padding:16px;text-align:center;color:#1e2a1e">{{ __('home.sections.achievements.items.3') }}</div>
            </div>
        </div>
    </section>
    <section class="cta">
        <div class="cta-content">
            <h3 class="cta-title">{{ __('home.cta.title') }}</h3>
            <a href="{{ url('/login') }}" class="cta-btn" id="ctaBtn">{{ __('home.cta.button') }}</a>
        </div>
    </section>
        ...
        <div class="container" style="display:grid;grid-template-columns:1.5fr 1fr 1fr;gap:16px">
            <div>
                <div style="display:flex;align-items:center;gap:8px">
                    <img src="{{ asset('images/logo.jpeg') }}" alt="SOQIA Innovative Environmental Solutions" style="height:16px;width:auto;border-radius:4px">
            ...
            <div>
                <div style="font-weight:700;color:#203020">{{ __('home.tabs.labels.products') }}</div>
                <div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:8px">
                    <a href="{{ url('/') }}" style="color:#203020;text-decoration:none">{{ __('home.nav.home') }}</a>
                    <a href="{{ url('/awareness-content') }}" style="color:#203020;text-decoration:none">{{ __('home.nav.awareness') }}</a>
                    <a href="{{ url('/login') }}" style="color:#203020;text-decoration:none">{{ __('home.nav.login') }}</a>
                    @auth
                    <a href="{{ url('/dashboard') }}" style="color:#203020;text-decoration:none">{{ __('home.nav.dashboard') }}</a>
                    @endauth
                </div>
            </div>
            <div>
