...
<body class="bg-gray-50 text-gray-800">
    ...
    <div class="max-w-7xl mx-auto px-5 py-8">
        <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-primary-green">{{ __('dashboard.greeting') }} <span id="userName">أحمد علي</span> 👋</h1>
                <div class="mt-1 text-sm text-gray-600">{{ __('dashboard.today') }}: <span id="dateNow"></span></div>
            </div>
            <div class="flex items-center gap-3">
                <a href="#" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-100 transition"><span>⚙️</span><span>{{ __('dashboard.actions.settings') }}</span></a>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-red-500 text-white hover:bg-red-600 transition"><span>⎋</span><span>{{ __('dashboard.actions.logout') }}</span></button>
                </form>
            </div>
        </div>
        ...
        <div class="mt-8 grid grid-cols-1 md:grid-cols-12 gap-6">
            <aside class="md:col-span-3">
                <div class="bg-white border border-gray-200 rounded-xl p-4 animate-in">
                    <div class="text-lg font-semibold text-primary-green">{{ __('dashboard.aside.title') }}</div>
                    <nav class="mt-3 space-y-2">
                        <a href="#" class="block rounded-lg px-3 py-2 bg-primary-green/10 text-primary-green">{{ __('dashboard.aside.dashboard') }}</a>
                        <a href="{{ url('/awareness-content') }}" class="block rounded-lg px-3 py-2 hover:bg-gray-100 transition">{{ __('dashboard.aside.awareness') }}</a>
                        <a href="#" class="block rounded-lg px-3 py-2 hover:bg-gray-100 transition">{{ __('dashboard.aside.my_consults') }}</a>
                        <a href="#" class="block rounded-lg px-3 py-2 hover:bg-gray-100 transition">{{ __('dashboard.aside.saved') }}</a>
                        <a href="#" class="block rounded-lg px-3 py-2 hover:bg-gray-100 transition">{{ __('dashboard.aside.settings') }}</a>
                    </nav>
                </div>
            </aside>
            ...
            <main class="md:col-span-9">
                <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
                    <div class="bg-white border border-gray-200 rounded-xl p-4 hover:shadow-md transition animate-in">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="badge bg-primary-blue/10 text-primary-blue">{{ __('dashboard.stats_cards.content') }}</div>
                        </div>
                        <div class="mt-3 text-sm text-gray-500">{{ __('dashboard.stats_cards.viewed') }}</div>
                        <div id="statViewed" class="mt-1 text-2xl font-extrabold text-gray-900">15</div>
                        <div class="mt-1 text-xs text-gray-500">{{ __('dashboard.stats_cards.last_update') }}</div>
                    </div>
                    <div class="bg-white border border-gray-200 rounded-xl p-4 hover:shadow-md transition animate-in">
                        <div class="flex items-center justify-between">
                            <div class="text-2xl">❤️</div>
                            <div class="badge bg-pink-100 text-pink-600">{{ __('dashboard.stats_cards.favorite') }}</div>
                        </div>
                        <div class="mt-3 text-sm text-gray-500">{{ __('dashboard.stats_cards.saved') }}</div>
                    ...
                    <div class="bg-white border border-gray-200 rounded-xl p-4 hover:shadow-md transition animate-in">
                        <div class="flex items-center justify-between">
                            <div class="text-2xl">📋</div>
                            <div class="badge bg-green-100 text-green-700">{{ __('dashboard.stats_cards.consultations') }}</div>
                        </div>
                        <div class="mt-3 text-sm text-gray-500">{{ __('dashboard.stats_cards.active') }}</div>
                    ...
                    <div class="bg-white border border-gray-200 rounded-xl p-4 hover:shadow-md transition animate-in">
                        <div class="flex items-center justify-between">
                            <div class="text-2xl">🏆</div>
                            <div class="badge bg-yellow-100 text-yellow-700">{{ __('dashboard.stats_cards.achievements') }}</div>
                        </div>
                        <div class="mt-3 text-sm text-gray-500">{{ __('dashboard.stats_cards.achievements') }}</div>
                        <div id="statAchievements" class="mt-1 text-2xl font-extrabold text-gray-900">5</div>
                        <div class="mt-1 text-xs text-gray-500">{{ __('dashboard.stats_cards.achievements_label') }}</div>
                    </div>
                </div>

                <div class="mt-6 bg-white border border-gray-200 rounded-xl p-4 animate-in">
                    <div class="flex items-center justify-between">
                        <h2 class="text-lg font-semibold text-gray-800">{{ __('dashboard.chart.title') }}</h2>
                    </div>
                    <div class="mt-3">
                ...
                <div class="mt-6 bg-white border border-gray-200 rounded-xl p-4 animate-in">
                    <div class="flex items-center justify-between">
                        <h2 class="text-lg font-semibold text-gray-800">{{ __('dashboard.active_consults.title') }}</h2>
                    </div>
                    <div class="mt-3 overflow-x-auto">
                        <table class="min-w-full text-sm">
                            <thead>
                                <tr class="text-gray-600">
                                    <th class="px-3 py-2 text-right">{{ __('dashboard.active_consults.table.name') }}</th>
                                    <th class="px-3 py-2 text-right">{{ __('dashboard.active_consults.table.date') }}</th>
                                    <th class="px-3 py-2 text-right">{{ __('dashboard.active_consults.table.status') }}</th>
                                    <th class="px-3 py-2 text-right">{{ __('dashboard.active_consults.table.action') }}</th>
                                </tr>
                            </thead>
                            <tbody id="consultationsTable">
                                <tr class="border-t">
                                    <td class="px-3 py-2">{{ app()->getLocale() === 'en' ? 'Feasibility study for a tomato farm' : 'دراسة جدوى لمزرعة الطماطم' }}</td>
                                    <td class="px-3 py-2">2024-01-15</td>
                                    <td class="px-3 py-2"><span class="badge bg-yellow-100 text-yellow-700">{{ __('dashboard.active_consults.status_pending') }}</span></td>
                                    <td class="px-3 py-2"><a href="#" class="text-primary-blue hover:underline">{{ __('dashboard.active_consults.view') }}</a></td>
                                </tr>
                                <tr class="border-t">
                                    <td class="px-3 py-2">{{ app()->getLocale() === 'en' ? 'Drip irrigation system consultation' : 'استشارة نظام الري بالتنقيط' }}</td>
                                    <td class="px-3 py-2">2024-01-10</td>
                                    <td class="px-3 py-2"><span class="badge bg-blue-100 text-blue-700">{{ __('dashboard.active_consults.status_in_progress') }}</span></td>
                                    <td class="px-3 py-2"><a href="#" class="text-primary-blue hover:underline">{{ __('dashboard.active_consults.view') }}</a></td>
                                </tr>
                            </tbody>
                        </table>
                ...
                <div class="mt-6 bg-white border border-gray-200 rounded-xl p-4 animate-in">
                    <div class="flex items-center justify-between">
                        <h2 class="text-lg font-semibold text-gray-800">{{ __('dashboard.saved_section.title') }}</h2>
                        <a href="#" class="text-primary-orange hover:underline">{{ __('dashboard.saved_section.view_all') }}</a>
                    </div>
                        <div class="mt-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            <div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
                            <div class="text-sm text-gray-900 font-semibold">{{ app()->getLocale() === 'en' ? '10 Tips to Save Water' : '10 نصائح لتوفير المياه' }}</div>
                            <div class="mt-1 text-xs text-gray-500">2024-01-12</div>
                            </div>
                            <div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
                            <div class="text-sm text-gray-900 font-semibold">{{ app()->getLocale() === 'en' ? 'Smart Irrigation System Explained' : 'شرح نظام الري الذكي' }}</div>
                            <div class="mt-1 text-xs text-gray-500">2024-01-11</div>
                            </div>
                            <div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
                            <div class="text-sm text-gray-900 font-semibold">{{ app()->getLocale() === 'en' ? 'Irrigation Systems Maintenance Guide' : 'دليل صيانة أنظمة الري' }}</div>
