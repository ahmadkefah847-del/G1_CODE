...
<body class="min-h-screen bg-gray-50 text-gray-800">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 min-h-screen">
        <div class="flex flex-col justify-center px-8 py-10 md:px-12">
            ...
            <div id="alert-success" class="hidden mb-4 rounded-md border border-green-200 bg-green-50 px-4 py-3 text-green-800"><?php echo e(__('login.messages.login_success')); ?></div>
            <div id="alert-error" class="hidden mb-4 rounded-md border border-red-200 bg-red-50 px-4 py-3 text-red-800"><?php echo e(__('login.messages.login_error')); ?></div>

            <form id="loginForm" class="space-y-4" method="post" action="">
                <?php echo csrf_field(); ?>
                <div class="relative">
                    <span class="input-icon">✉️</span>
                    <input id="email" type="email" placeholder="<?php echo e(__('login.form.email')); ?>" class="w-full rounded-lg border border-gray-300 bg-white pr-9 py-3 focus:border-primary-green focus:ring-2 focus:ring-primary-green/20 transition" autocomplete="username">
                ...
                <div class="relative">
                    ...
                    <input id="password" type="password" placeholder="<?php echo e(__('login.form.password')); ?>" class="w-full rounded-lg border border-gray-300 bg-white pr-9 pl-9 py-3 focus:border-primary-green focus:ring-2 focus:ring-primary-green/20 transition" autocomplete="current-password" minlength="6">
                ...
                <div class="flex items-center justify-between">
                    <label class="flex items-center gap-2 text-sm text-gray-700">
                        <input id="remember" type="checkbox" class="h-4 w-4 rounded border-gray-300 text-primary-green focus:ring-primary-green">
        ...
        <div class="hidden md:block relative">
            ...
            <img src="<?php echo e(asset('images/farm1.jpg')); ?>" alt="login background" class="w-full h-full object-cover">
        </div>
    </div>
    ...
    <script>
        ...
        const TEST_ACCOUNTS = [
            { email: 'admin@saqqia.jo', password: 'password123' },
            { email: 'farmer@saqqia.jo', password: 'password123' }
        ];
        ...
        form.addEventListener('submit', (e) => {
            ...
            setTimeout(() => {
                const match = TEST_ACCOUNTS.some(acc => acc.email === email.value && acc.password === password.value);
                if(match){
                    alertSuccess.textContent = T.success;
                    alertSuccess.classList.remove('hidden');
                    alertError.classList.add('hidden');
                } else {
                    alertError.textContent = T.error;
                    alertError.classList.remove('hidden');
                    alertSuccess.classList.add('hidden');
                }
...
<?php /**PATH C:\Users\a3m20\CascadeProjects\saqqia\resources\views/auth/login.blade.php ENDPATH**/ ?>